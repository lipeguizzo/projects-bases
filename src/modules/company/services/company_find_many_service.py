...
class CompanyFindManyService:
    @classmethod
    def execute(self, dto: CompanyFindManyDTO, user_request: UserRequest):

        [companies, total_companies] = self.find_companies(dto, user_request)
    ...
    @classmethod
    def find_companies(self, params: CompanyFindManyDTO, user_request: UserRequest):
        ...
        companies = Company.objects.filter(dynamic_where).order_by(order_by)[
            skip : skip + take
        ]
    ...
    def generate_dynamic_where(
        self, params: CompanyFindManyDTO, user_request: UserRequest
    ):
        ...
        if user_request.company:
            dynamic_where &= Q(id=user_request.company.id)
    ...
    @classmethod
    def format_order_by_column(self, params: CompanyFindManyDTO):
